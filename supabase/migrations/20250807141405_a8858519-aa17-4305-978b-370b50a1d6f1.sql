-- ===================================
-- STORYCLASH ENHANCED SYSTEM SCHEMA
-- ===================================
...
CREATE TABLE IF NOT EXISTS competitor_campaigns (
  ...
  content_themes text[],
  hashtags text[],
  detected_by text, -- manual, api, scraping
  created_at timestamp with time zone DEFAULT now(),
  updated_at timestamp with time zone DEFAULT now()
);

-- Team roles and permissions for campaigns
CREATE TABLE IF NOT EXISTS campaign_team_members (
  id uuid PRIMARY KEY DEFAULT gen_random_uuid(),
  campaign_id uuid REFERENCES campaigns(id) ON DELETE CASCADE,
  user_id uuid,
  role text NOT NULL, -- admin, creative, strategy, account_manager
  permissions text[],
  added_by uuid,
...
-- Product matching for lookalike engine
...
-- RLS Policies for campaign_stages
...
-- RLS Policies for brand_monitoring
...
CREATE POLICY "Users can view competitor campaigns" ON competitor_campaigns
  FOR SELECT USING (auth.role() = 'authenticated');

CREATE POLICY "Users can manage competitor campaigns" ON competitor_campaigns
  FOR ALL USING (auth.role() = 'authenticated');

-- RLS Policies for campaign_team_members
CREATE POLICY "Users can view campaign team for their campaigns" ON campaign_team_members
  FOR SELECT USING ((SELECT organization_id FROM campaigns WHERE id = campaign_team_members.campaign_id) = get_user_organization_id() OR is_user_admin());

CREATE POLICY "Users can manage campaign team for their campaigns" ON campaign_team_members
  FOR ALL USING ((SELECT organization_id FROM campaigns WHERE id = campaign_team_members.campaign_id) = get_user_organization_id() OR is_user_admin());
